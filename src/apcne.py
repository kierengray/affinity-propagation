...
def main():
	df= pd.read_csv('cne.csv', header=0)
	names=df[df.columns[0]].tolist()
	df.drop(df.columns[0], axis=1, inplace=True)
	maxits=	500	#Algorithm terminates when maxits reached unless convits reached before.
	convits=50 #Algorithm terminates if exemplars unchanged of convits iterations.
...
def results(assignment, names, df):
	...
	with open("apcneresults.csv", "wb") as f:
		writer = csv.writer(f)
		writer.writerows(results)

	print metrics.silhouette_score(df, assignment, metric='euclidean')
	return
